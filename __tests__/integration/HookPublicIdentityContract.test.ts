...
describe("Hook public identity contract", () => {
    it("uses a public GitHub-backed Tauri identifier without changing the visible product name", () => {
        expect(tauriConfig.identifier).toBe("io.github.aiaimimi0920.hook");
        expect(tauriConfig.productName).toBe("hook");
    });

    it("keeps a legacy app-data fallback so older local installs retain user state", () => {
        expect(rustSource).toContain('const LEGACY_TAURI_IDENTIFIER: &str = "com.vmjcv.hook";');
