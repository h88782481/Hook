...
fn overlay_image(
    ...
    let src = rgba.as_raw();
    let total_height = if total_width == 0 { 0 } else { final_image.len() / (total_width * channel_count) };
    for y in 0..height {
        let dy = offset_y.wrapping_add(y);
        if dy >= total_height {
            continue;
        }
        for x in 0..width {
            let dx = offset_x.wrapping_add(x);
            if dx >= total_width {
                continue;
            }
            let dst = (dy * total_width + dx) * channel_count;
            let src_i = (y * width + x) * 4;
            if dst + 3 < final_image.len() && src_i + 3 < src.len() {
                final_image[dst..dst + 4].copy_from_slice(&src[src_i..src_i + 4]);
            }
        }
    }
}
