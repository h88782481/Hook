...
use crate::{CursorPosition, DragItem, DragMode, DragResult, DropOutcome, Error, Image, Options};
use gdkx11::{
    gdk,
...
pub fn start_drag<F: Fn(DropOutcome) + Send + 'static>(
    ...
    let drag_action = match options.mode {
        ...
        DragMode::CopyOrMove => gdk::DragAction::COPY | gdk::DragAction::MOVE,
    };
...
fn on_drop_failed<F: Fn(DropOutcome) + Send + 'static>(
    callback: Rc<F>,
    window: &gtk::ApplicationWindow,
    ...
    handler_ids
        ...
        .push(window.connect_drag_failed(move |_, _, _drag_result| {
            ...
            callback(DropOutcome {
                result: DragResult::Cancel,
                cursor_position: get_cursor_position(&window_clone).unwrap(),
...
fn on_drop_performed<F: Fn(DropOutcome) + Send + 'static>(
    callback: Rc<F>,
    window: &gtk::ApplicationWindow,
    handler_ids: &Arc<Mutex<Vec<SignalHandlerId>>>,
    ...
    drag_context.connect_drop_performed(move |context, _| {
        ...
        callback(DropOutcome {
            ...
            cursor_position: get_cursor_position(&window).unwrap(),
            performed_effect: None,
            platform_status: None,
        });
    });
