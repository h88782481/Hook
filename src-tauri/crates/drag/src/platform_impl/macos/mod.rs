...
type OnDropCallback = Box<dyn Fn(DropOutcome) + Send>;
...
define_class!(
    ...
    unsafe impl NSDraggingSource for DragRsSource {
        ...
        unsafe fn dragging_session_end(
            ...
            callback_closure(DropOutcome {
                result: if operation == objc2_app_kit::NSDragOperation::None {
                    DragResult::Cancel
                } else {
                    DragResult::Dropped
                },
                cursor_position: mouse_location,
...
impl DragRsSource {
    pub fn new<F: Fn(DropOutcome) + Send + 'static>(
        on_drop_callback: F,
        options: &Options,
